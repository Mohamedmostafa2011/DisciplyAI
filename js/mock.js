...
import { todayISO, addDays, label, longLabel, parseDate, parseRange } from './dates.js';
...
const DB = {
  // Subject values used by the Subject column on Tasks & Homework
...
export async function notionStatus() {
  await wait(420);
  return { ok: true, demo: true, workspace: 'Disciplay (sample data)', databases: 2 };
}
...
export async function sendChat({ messages, confirmedAction }, onEvent) {
  ...
  if (/\bplan\b/.test(t) && /\bweek|day|study\b/.test(t)) {
    ...
    return { reply: `Here's your week, built from Notion:\n\n${body}\n\n---\n\n**Suggestion:** start with **${upcoming[0].subject} — ${upcoming[0].title}** today, and keep the heavier subjects for when you have most energy.`, tools };
  }

  /* ---- 6. Subjects (values on the Subject column) ---- */
  if (/\bsubject/.test(t)) {
    await run('get_homework', () => ({ done: 'Done — homework loaded.' }));
    await run('get_tasks', () => ({ done: 'Done — tasks loaded.' }), 380);
    const used = [...new Set([...DB.homework, ...DB.tasks].map((i) => i.subject).filter(Boolean))].sort();
    return { reply: `These are the subjects currently used across your Tasks and Homework:\n\n${used.map((s) => `- ${s}`).join('\n')}`, tools };
  }

  /* ---- 7. Out of scope (no commitments/timetable database) ---- */
  if (/\bcommitment|timetable|fixed schedule|lesson|class time\b/.test(t)) {
    return { reply: `I only manage your **Tasks** and **Homework** databases in Notion — I don't have a timetable or commitments database connected.\n\nI can still help you with:\n\n- What's due today or this week\n- Adding homework and tasks\n- Rescheduling or marking things done\n- Planning your week around your deadlines`, tools };
  }

  /* ---- 8. READ tasks / homework ---- */
