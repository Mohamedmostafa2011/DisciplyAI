...
export default async function handler(req, res) {
  ...
  // Does the key look like it belongs to the configured provider?
  if (aiKey) {
    const isGroq = base.includes('api.groq.com');
    if (isGroq && !aiKey.startsWith('gsk_')) {
      add('Key matches provider', false,
          `AI_BASE_URL points at Groq but the key does not start with "gsk_". A Groq key looks like gsk_… — an OpenAI key (sk-…) will be rejected with 401.`);
    } else {
      add('Key matches provider', true, isGroq ? 'Groq key format (gsk_…)' : 'custom provider');
    }
  }
