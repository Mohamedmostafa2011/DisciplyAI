...
function secret() {
  ...
  return cleanEnv(process.env.SESSION_SECRET, 'SESSION_SECRET') || crypto.createHash('sha256')
    .update(cleanEnv(process.env.DISCIPLAY_PASSWORD, 'DISCIPLAY_PASSWORD') || 'disciplay-dev-secret')
    .digest('hex');
}
...
export function checkPassword(input) {
  const expected = cleanEnv(process.env.DISCIPLAY_PASSWORD, 'DISCIPLAY_PASSWORD');
