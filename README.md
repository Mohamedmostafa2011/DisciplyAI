...
information stored in **Notion databases**. You talk to it normally — it reads, creates, updates and
deletes homework, tasks, subjects and fixed commitments on your behalf.
...
- 📓 **Full CRUD on Notion** — read, create, update and delete homework, tasks, subjects and commitments
...
- 📱 **Responsive & accessible** — 320px → 1920px+, keyboard navigation, ARIA labels, focus states
...
the `NOTION_DB_*` environment variables.

## Environment variables

Create these on your host (Vercel → *Project Settings → Environment Variables*,
Netlify → *Site settings → Environment variables*, or a local `.env` file).
...
| `NOTION_DB_COMMITMENTS` | – | Fixed Commitments database ID |
| `NOTION_DB_SUBJECTS` | – | Subjects database ID |
...
| --- | --- | --- | --- |
| **Groq** (default) | `https://api.groq.com/openai/v1` | `llama-3.3-70b-versatile` | ✅ Free tier, no card |
...
```
get_tasks   get_homework   get_fixed_commitments   get_subjects
